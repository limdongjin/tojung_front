<template>
    ...
    <div class="pro-info-xs visible-xs">
        <h4 class="font-18 font-weight-bold"><%= @product.name %></h4>
    ...
    <div class="d-flex visible-lg py-3 pro-info-wrapper justify-content-center">
        ...
        <div class="pro-info">
            ...
            <div class="dropdown my-3 w-100" id="myDropdown">
                <button type="button" id="packageButton"
                ...
                <ul class="dropdown-menu w-100 py-0" aria-labelledby="packageButton">
                    ...
                    <li class="d-flex package-box" onclick="packageDropdown(this, <%= @product.id %>);">
                        <%end%>
                        <%=image_tag(@cdn_url + package.image0.path, class: "package-item mx-3 my-3 hide", id:
                        package.id) if package.image0.path != nil%>
            ...
            <div class="buttons">
                <div class="d-flex">
                    <div class="dropdown dropdown-share">
                        <button class="share-button dropdown-toggle" data-toggle="dropdown" aria-haspopup="true"
                                aria-expanded="false">공유하기
                        </button>
                        <ul class="dropdown-menu" id="shareDropdown">
                            <li class="d-block">
                                <a id="kakao-link-btn" href='javascript:createlink("<%= @product.name.gsub("'", "")
                    ...
                    <div class="dropdown-share" id="dropdownShare">
                        ...
                        <button class="share-button heart" id="Heart"><%=image_tag("heart.svg", class: "heart-img")%>
                            찜하기
                        </button>
                        <% end %>
                        <% elsif @like.empty? %>
                        ...
                        <%= hidden_field_tag 'product_id', @product.id%>
                        <button class="share-button heart" id="Heart"><%=image_tag("heart-red.svg", class:
                            "heart-img")%> 찜 취소하기
                        </button>
                        <% end %>
                        ...
                        <%= form_tag "/product_like/#{ @like[0].id }", remote: true, method: 'put' do %>
                        <%= hidden_field_tag 'product_id', @product.id%>
                        <button class="share-button heart" id="Heart" onclick=""><%=image_tag("heart.svg", class:
                            "heart-img")%> 찜하기
                        </button>
                        <% end %>
                        <%end%>
